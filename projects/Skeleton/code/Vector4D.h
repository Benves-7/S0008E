...
public:
	...
	static Vector4D vLerp(Vector4D first, Vector4D second, float step)
	{
		return Vector4D(
			...
			internalLerp(first[1], second[1], step),
	...
	{
		return Vector4D(this->cord[0] * rhs, this->cord[1] * rhs, this->cord[2] * rhs, 1);
	}
	Vector4D operator+(Vector4D& rhs)
	{
		return Vector4D((this->cord[0] + rhs.cord[0]), (this->cord[1] + rhs.cord[1]), (this->cord[2] + rhs.cord[2]));
	}
	Vector4D operator-(Vector4D& rhs)
	{
		return Vector4D((this->cord[0] - rhs.cord[0]), (this->cord[1] - rhs.cord[1]), (this->cord[2] - rhs.cord[2]));
...
private:
	float cord[4];
	static float internalLerp(float first, float second, float step)
	{
