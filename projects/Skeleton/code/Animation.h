...
public:
	...
	bool loadAnimations(char* filename) {
        ...
        header = (Nax3Header*)ptr;
        if(header->magic != NAX3_MAGICNUMBER)
            return false;

        keyBuffer = (Vector4D*)malloc(header->numKeys * sizeof(Vector4D));
        ptr += sizeof(Nax3Header);

        clips = new Clip[header->numClips];
